<template>
  <div class="create-cursor-rule">
    <div class="container">
      <div class="form-container">
        <div class="form-header">
          <h1>创建 Cursor Rule</h1>
          <p>创建一个新的 cursor rule 并分享给其他开发者</p>
        </div>
...
import { 
  QuestionFilled, MagicStick, Delete, View, Upload 
...
const clearContent = async () => {
  try {
    await ElMessageBox.confirm('确定要清空内容吗？', '警告', {
...
const handleSubmit = async () => {
  try {
    ...
    // 创建cursor rule
    const response = await api.post('/cursor-rules', submitData)
    
    ElMessage.success('规则创建成功！')
    
    // 跳转到详情页
    router.push(`/cursor-rule/${response.data.data.cursor_rule_id}`)
